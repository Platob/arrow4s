...
trait ToValueConverter {
  implicit def toBytes[T](implicit codec: ValueCodec[T]): ValueConverter[T, Array[Byte]] = {
    new ValueConverter[T, Array[Byte]] {
      ...
      override val target: ValueCodec[Array[Byte]] = ValueCodec.binary
  ...
  implicit def toString[T](implicit codec: ValueCodec[T]): ValueConverter[T, String] = {
    new ValueConverter[T, String] {
      override val source: ValueCodec[T] = codec
      override val target: ValueCodec[String] = ValueCodec.utf8
      override def decode(value: String): T = source.fromString(value)
      override def encode(value: T): String = source.toString(value)
    }
  }

  implicit def toBoolean[T](implicit codec: ValueCodec[T]): ValueConverter[T, Boolean] = {
    new ValueConverter[T, Boolean] {
      override val source: ValueCodec[T] = codec
      override val target: ValueCodec[Boolean] = ValueCodec.boolean
  ...
  implicit def toOption[F, T](implicit
    vc: ValueConverter[F, T], toOpt: OptionalValueCodec[T]
  ): ToValueConverter.ToOption[F, T] =
    new ToValueConverter.ToOption[F, T](vc, toOpt)
}
